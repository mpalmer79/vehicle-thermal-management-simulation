...
LOCK_PATH = ROOT / "validation_configs" / "argonne_2012_focus_val_hot_01_manifest.json"
...
def run(source_path: Path) -> dict[str, object]:
    ...
    if manifest.permits_parameter_fitting:
        raise RuntimeError("VAL-HOT-01 must never permit parameter fitting")
    ...
    if parameter_sha != manifest.parameter_snapshot_sha256:
        raise RuntimeError("VAL-HOT-01 final staged parameter snapshot does not match the frozen manifest")
    ...
    if signal_map_sha != manifest.preprocessing_snapshot_sha256:
        raise RuntimeError("VAL-HOT-01 signal-map snapshot does not match the frozen manifest")
    ...
    if dataset.metadata.get("source_sha256") != manifest.dataset_fingerprint.sha256_hex:
        raise RuntimeError("VAL-HOT-01 source file SHA-256 does not match the frozen manifest")
    if int(dataset.metadata.get("source_size_bytes", -1)) != manifest.dataset_fingerprint.size_bytes:
        raise RuntimeError("VAL-HOT-01 source file size does not match the frozen manifest")
    ...
    return {
        ...
        "interpretation": {
            ...
            "formal_validation_claim_allowed_only_if_acceptance_formal_validation_pass_is_true": True,
        },
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Execute the locked VTMS VAL-HOT-01 Argonne independent holdout."
    )
    parser.add_argument("source", type=Path, help="Path to the original 71207063 Test Data.txt file")
