...
const STATUS_OVERRIDES: Record<string, Pick<KnowledgeTopic, "shortAnswer" | "detail">> = {
  "verification-and-validation": {
    shortAnswer:
      "The two remain deliberately separate. Numerical verification is complete. Argonne controlled calibration has been run and frozen, and the primary independent physical holdout now has a formal failure result. VTMS-V1 is not physically validated.",
    detail: [
      ...
      "VAL-HOT-01 test 71207063 was evaluated as the untouched primary physical holdout with the final staged parameter snapshot. It failed formal acceptance. No holdout-driven retuning is permitted.",
    ],
  },
  argonne: {
    shortAnswer:
      "Argonne supplied controlled 2012 Ford Focus D3 data. Results now exist, so the earlier status 'no Argonne results exist yet' is superseded. The staged calibration is frozen and the untouched hot-start holdout 71207063 produced a formal failure: RMSE about 8.59 °C, MAE 8.07 °C, mean bias -8.04 °C, and P90 absolute error 10.05 °C. VTMS-V1 did not pass controlled physical validation.",
    detail: [
  ...
  "validation-status": {
    shortAnswer:
      "VTMS-V1 is numerically verified but not physically validated. The controlled Argonne calibration sequence is frozen, and the primary independent physical holdout failed the preregistered acceptance criteria.",
    detail: [
      ...
      "The untouched hot-start holdout produced RMSE 8.59 °C, MAE 8.07 °C, mean bias -8.04 °C, and P90 absolute error 10.05 °C.",
