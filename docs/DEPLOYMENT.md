...
## JavaScript dependency reproducibility

The repository does not currently commit a `web/package-lock.json`. UI-3 therefore uses `npm install` rather than claiming deterministic lockfile-based installation. Production dependency security is gated separately with `npm audit --omit=dev --audit-level=high`, and weekly Dependabot monitoring is enabled.

A committed lockfile is a recommended future reproducibility improvement. It should be generated by the normal npm toolchain and reviewed as a real dependency artifact rather than manually fabricated.
