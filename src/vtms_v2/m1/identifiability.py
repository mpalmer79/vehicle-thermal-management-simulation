...
@dataclass(frozen=True)
class M1IdentifiabilityDiagnostic:
    case_ids: tuple[str, ...]
    parameter_names: tuple[str, ...]
    perturbation_fraction: float
    sample_count: int
    sensitivities: tuple[M1ParameterSensitivity, ...]
...
def _prediction(case, parameters: M1Parameters) -> np.ndarray:
    result = M1SimulationRunner(parameters).run(
        case.scenario,
...
def _perturbed_values(
    ...
    return theta * (1.0 - perturbation_fraction), theta * (1.0 + perturbation_fraction)


def evaluate_m1_identifiability(
    *,
    parameters: M1Parameters | None = None,
    parameter_names: Iterable[str] = M1_CANDIDATES,
    perturbation_fraction: float = 0.01,
    weak_relative_rms_threshold: float = 0.02,
    correlation_threshold: float = 0.95,
    condition_number_threshold: float = 100.0,
) -> M1IdentifiabilityDiagnostic:
    ...
    baseline = parameters or M1Parameters()
    baseline.validate()
    names = tuple(parameter_names)
    if not names:
        raise ValueError("at least one parameter name is required")
    unknown = [name for name in names if name not in M1_CANDIDATES]
    if unknown:
        raise ValueError(f"unsupported M1 identifiability parameters: {unknown}")
    ...
    for name in names:
        ...
        for case in cases:
            ...
            blocks.append((y_high - y_low) / (2.0 * perturbation_fraction))
    ...
    strongest_pairwise = float(np.max(off_diagonal)) if off_diagonal.size else 0.0
    weak_names = tuple(
        name
    ...
    note = (
        "Synthetic M1 subset is locally distinguishable under the frozen excitation. "
        "This is mathematical preflight only and does not authorize physical calibration."
        if authorized
        else
        "Synthetic M1 identifiability warning. Reduce the physical fitting stage or independently constrain weak/correlated parameters."
    )

    return M1IdentifiabilityDiagnostic(
...
def evaluate_m1_staged_identifiability(
    *,
    parameters: M1Parameters | None = None,
) -> dict[str, M1IdentifiabilityDiagnostic]:
    baseline = parameters or M1Parameters()
    return {
        "topology_only": evaluate_m1_identifiability(
            parameters=baseline,
            parameter_names=("hot_coolant_capacitance_fraction",),
        ),
        "topology_engine": evaluate_m1_identifiability(
            parameters=baseline,
            parameter_names=(
                "hot_coolant_capacitance_fraction",
                "engine_thermal_capacitance_j_per_k",
        ...
        "topology_heat_rejection": evaluate_m1_identifiability(
            parameters=baseline,
            parameter_names=(
                "hot_coolant_capacitance_fraction",
                "radiator_ua_nominal_w_per_k",
        ...
        "all_candidates": evaluate_m1_identifiability(
            parameters=baseline,
        ),
    }
