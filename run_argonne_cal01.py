...
from vtms_validation.physical_bounds import argonne_cal_01_bounds
...
def run(source_path: Path) -> dict[str, object]:
    ...
    if signal_map_sha != manifest.preprocessing_snapshot_sha256:
        raise RuntimeError("CAL-01 signal-map snapshot does not match the frozen manifest")
    ...
    if initial_parameter_sha != manifest.parameter_snapshot_sha256:
        raise RuntimeError("CAL-01 baseline parameter snapshot does not match the frozen manifest")

    bounds = argonne_cal_01_bounds()
    bounds_sha = sha256_mapping(bounds.as_dict())
    if bounds_sha != manifest.calibration_bounds_sha256:
        raise RuntimeError("CAL-01 staged bound snapshot does not match the frozen manifest")
    ...
    if dataset.metadata.get("source_sha256") != manifest.dataset_fingerprint.sha256_hex:
        raise RuntimeError("CAL-01 source file SHA-256 does not match the frozen manifest")
    if int(dataset.metadata.get("source_size_bytes", -1)) != manifest.dataset_fingerprint.size_bytes:
        raise RuntimeError("CAL-01 source file size does not match the frozen manifest")
    ...
    return {
        ...
        "calibration_stage": "CAL-01 cold-start warm-up",
        ...
        "initial_parameter_snapshot_sha256": initial_parameter_sha,
        ...
        "interpretation": {
            ...
            "effective_parameter_warning": (
                "CAL-01 parameters are effective VTMS-V1 lumped values. Strong sensitivity correlation "
                "means individual fitted values must not be reported as uniquely identified physical properties."
            ),
            "next_stage_if_frozen": (
                "Freeze the CAL-01 parameter snapshot before CAL-RAD-01. CAL-RAD-01 may move only radiator UA."
            ),
        },
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Execute the locked VTMS CAL-01 Argonne calibration against a local source file."
    )
    parser.add_argument("source", type=Path, help="Path to the original 71207062 Test Data.txt file")
